...
All coordinate data are saved in http://gisgeography.com/wgs84-world-geodetic-system/[WGS84] horizontal spatial reference system.  The elevation data saved by Geopaparazzi is as reported by the GPS library for the device and is likely the height relative to the WGS84 ellipsoid and not a vertical datum or some other spatially varying surface like Mean Sea Level.
