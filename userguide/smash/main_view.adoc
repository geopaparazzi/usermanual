...
_By double tapping the GPS button the automatic map centering on GPS is toggled._

On **Android** devices the GPS position notification message is dynamic. In the notification of the device there are all the information about the GPS status (position and logging).

[.text-center]
====
image::images/version_1_3_0/gps_notification_01.jpg[scaledwidth=50%, width=50%]
The visualization of the GPS notification on an _Android_ device.
====
...
Adding simple notes to the project. The note will be placed in the current center of the map.
====
...
[.text-center]
====
image::images/version_1_3_0/gps_notification_02.jpg[scaledwidth=50%, width=50%]
The visualization of the GPS notification on an _Android_ device.
====
...
[.text-center]
====
image::images/version_1_3_0/18_toolbar.jpeg[scaledwidth=60%, width=60%]
The bottom toolbar handler in the map view.
====

[.text-center]
====
image::images/version_1_3_0/19_toolbar.jpeg[scaledwidth=60%, width=60%]
...
The first icon of the bottom toolbar represents the feature editing tool for vector layers in the Geopackage format:

* of all feature's type: point, lines, polygons and multipoint, multiline, multipolygon
* in their own coordinate reference system.

The features editing works for all visible vector layers of a Geopackage  and allows to:
...
[.text-center]
====
image::images/version_1_3_0/36_editing.jpeg[scaledwidth=30%, width=30%]
The moment of tapping and loading query data with enabled the features editing tool.
====

Once activated **long tap** on a feature in the map to select the feature below, for polygons, or in the very near, for lines and points. The diameter of influence of the tapped position can be set in the general _Settings -> Vector layers -> Info Tool_. 

The selected feature will be highlighted in yellow in the map and some specific editing tools will be displayed in the bottom toolbar to:

1. delete
2. edit the attributes table
3. save the modified geometry
4. cancel the geometry changes until last saved action.

[.text-center]
====
image::images/version_1_3_0/37_editing.jpeg[scaledwidth=30%, width=30%]
The long tapped polygon feature selected.
====

==== Editing of existing features 

Editing of an existing geometry consists of:

a) drag edges/angles in the new position

b) move the middle points of segments: this will become an edge and automatically two additional middle points will be added from the selected vertex to the two adjacient vertexes in both directions

c) long tap on a node to remove it.

[.text-center]
====
image::images/version_1_3_0/38_editing.jpeg[scaledwidth=30%, width=30%]
An example of an edited polygon feature.
====

As soon as the modified geometry is ready, it is possible to either save the changes, by tapping on the save icon  image:images/version_1_3_0/39_editing.png[scaledwidth=5%, width=5%] or long tap on the cancel icon image:images/version_1_3_0/40_editing.png[scaledwidth=5%, width=5%] to restore the initial geometry for that feature. Both the commands will close the editing tool for the current geometry.
...
To exit the tool tap the left arrow on the top left of the window.

[.text-center]
====
image::images/version_1_3_0/44_editing.jpeg[scaledwidth=30%, width=30%]
Editing attributes of a polygon layer.
====


==== Add new features 

To add a new feature in one of the visible Geopackage layers long tap on the desidered position regardless of the type of the new feature you want to add:

a) the application will ask in which layer to save the new feature 

[.text-center]
====
image::images/version_1_3_0/41_editing.jpeg[scaledwidth=30%, width=30%]
The list of the available layers where to create the new feature.
==== 

b) select the layers from the list of the available 

c) new feature of the type of the selected layer will be added automatically in the tapped position:

    * point layers: a new point is added in the tapped position
    * line layers: a small line with two vertexes will be added in the tapped position, the user can then modify this geometry following the _Editing of existing features_ instructions
    
[.text-center]
====
image::images/version_1_3_0/42_editing.jpeg[scaledwidth=30%, width=30%]
An example of the new line feature created automatically by the application.
====    
    
    * polygon layers: a small polygon with 3 vertexes will be added in the tapped position, the user can then modify this geometry following the _Editing of existing features_ instructions

[.text-center]
====
image::images/version_1_3_0/43_editing.jpeg[scaledwidth=30%, width=30%]
An example of the new polygon feature created automatically by the application.
====
...
* long tap on the feature to delete to select it

* tap the trash icon in the bottom toolbar.

NOTE:: please consider that the delete tool has no possibility to reverse the action, there are no _undo_ or _cancel_ tools working after it.
...
The info tool queries all visible vector layers and opens the feature info view, which showsdddddd each selected feature highlighted in the map context and its attributes:

* to browse between the selecte features (if more than one) use the arrows on the top right of the window
* it is possible to edit the attributes of the feature only of Geopackage layers by selecting the pencil near the value of the attribute in the table

[.text-center]
====
image::images/version_1_3_0/21_query_layers.jpeg[scaledwidth=30%, width=30%]
The feature info view showing a polygon feature in a non Geopackage layer.
====

[.text-center]
====
image::images/version_1_3_0/22_query_layers.jpeg[scaledwidth=30%, width=30%]
The feature info view showing a point feature in a Geopackage layer with the possibility for attribute's editing.
====
...
[.text-center]
====
image::images/version_1_3_0/20_measure.jpeg[scaledwidth=50%, width=50%]
The measurement tool.
====
...
When tapping on the **Project** action, the user is prompted to the starting panel (Project view) where it is possible to _Open and existing project_, _Create a new project_ :
...
or select one of the most _Recent projects_ from the list.
...
In fact SMASH is currently the suggested tool to synchronize Geopaparazzi and SMASH surveys with the GSS.
...
Using the left icon, data can be downloaded to the device. Once downloaded the application will show what datasets are still available.

=== Export

SMASH is currently able to export to files and to synchronize the project with the https://www.geopaparazzi.org/gss/index.html[Geopaparazzi Survey Server].
...
When pushing the pdf export button, a pdf containing all the notes in the project (simple and form based), is generated and saved to the export folder in the application directory.

[.text-center]
====
image::images/version_1_3_0/pdf_export.jpeg[scaledwidth=70%, width=70%]
The export view.
====

==== GPX

When pushing the gpx export button, the application exports the whole survey project to a single GPX and saves the file in the export folder in the application directory.

==== KML

When pushing the kml export button, the application exports the whole survey project to a single KML and saves the file in the export folder in the application directory.

==== GEOPACKAGE

When pushing the Geopackage export button, the application exports the whole survey project to a single Geopackage and saves the file in the export folder in the application directory.

[.text-center]
====
image::images/version_1_3_0/geopkg_export.jpeg[scaledwidth=70%, width=70%]
The export view.
====
...
[.text-center]
====
image::images/ipad/gss_upload_error.jpeg[scaledwidth=60%, width=60%]
...
[.text-center]
====
image::images/ipad/settings.jpeg[scaledwidth=40%, width=40%]
The settings view.
====
...
[.text-center]
====
image::images/version_1_3_0/27_main_drawer.jpeg[scaledwidth=40%, width=40%]
...
There is also the possibility to visualize the GPS point count separated for the _valid_ and for _all_ points. And to decide to use the GPS filtered points (currently calculated applying the Kalman filter) to all GPS dependent features of the application (instead of the original raw data).

NOTE: In case of GPS logs original and filtered GPS signal are **BOTH** always saved into the database and the user can choose to look at the original and/or filtered log in the <<GPS Logging visualization>>.
...
[.text-center]
====
image::images/version_1_3_0/32_live_preview.jpeg[scaledwidth=40%, width=40%]
The gps settings view.
====
...
[.text-center]
====
image::images/version_1_3_0/28_main_drawer.jpeg[scaledwidth=40%, width=40%]
...
* define the color picker to use in the application when setting a color
...
[.text-center]
====
image::images/version_1_3_0/29_main_drawer.jpeg[scaledwidth=60%, width=60%]
...
* define the radius of the tap of the info feature and editing tool 
* set the dimension of the editing drag handler size
* set the dimension of the editing intermediate drag handler size.
...
[.text-center]
====
image::images/version_1_3_0/33_main_drawer.png[scaledwidth=60%, width=60%]
The CRS view.
====

[.text-center]
====
image::images/version_1_3_0/34_main_drawer.png[scaledwidth=60%, width=60%]
The add CRS panel.
====
...
The vector layers can be in different projections, they will be reprojected on the fly.
...
[.text-center]
====
image::images/version_1_3_0/main_view_mapsforge.jpeg[scaledwidth=70%, width=70%]
...
[.text-center]
====
image::images/version_1_3_0/sld_08_points_map.jpeg[scaledwidth=30%, width=30%]
...
[.text-center]
====
image::images/version_1_3_0/sld_12_theme_map.jpeg[scaledwidth=30%, width=30%]
...
[.text-center]
====
image::images/version_1_3_0/sld_15_gpx_map.jpeg[scaledwidth=30%, width=30%]
An example of the styling options of a GPX layer in SMASH.
====
...
The support for the visualization of the Geopackage raster layers allows to set different parameters like the tiling system and the projection. 
As for the tiling system we reccommand to use Gdal to create Geopackage rasters. In fact, Gdal by default generates only one zoomlevel at the optimal image resolution and using the best tile schema. This allows to create geopackages of big imagery maintaining more or less the same size o the original data.

Smash supports different projections and does the reprojection on the flight if it is different from the usual EPSG 3857 used as base for the application. 

Geopackage tile datasets can be loaded in two different ways. The settings are available in the properties view of geopackage layers.

[.text-center]
====
image::images/version_1_5_0/01_gpkg_props.png[scaledwidth=60%, width=60%]
The Layers view: a detail of possibilities for a Geopackage layer.
====


[.text-center]
====
image::images/version_1_5_0/02_gpkg_props.png[scaledwidth=60%, width=60%]
The properties view of a geopackage layer.
====

The available options are:

* the opacity value of the entire map
* the possibility to load a wider range of tiles schemas: to use for an ortophoto reprojected with gdal and packaged as geopackage

[.text-center]
====
image::images/version_1_5_0/03_gladschema.png[scaledwidth=70%, width=70%]
The visualization of a geopackage layer with an ortophoto reprojected with gdal.
====

NOTE: This feature has a larger memory footprint as it loads the tiles in memory as an image. So if the dataset is too large for your device it will not work and it has to be disabled.


* the color to make transparent: this is useful when there is the need to overlay technical maps over an elevation model or ortophoto. But it also solves a situation like this:

[.text-center]
====
image::images/version_1_5_0/04_collar.png[scaledwidth=70%, width=70%]
The visualization of two geopackage layers with an ortophoto reprojected with gdal, note the black collar.
====

where there are two geopackage layers of a reprojected ortophoto. When both loaded the black collar of one covers the other. It is possible to set the black collar to transparent by selecting the black color in the picker.

[.text-center]
====
image::images/version_1_5_0/06_collar_fixtile.png[scaledwidth=70%, width=70%]
The visualization of two geopackage layers with an ortophoto reprojected with gdal, with the black color set to transparent.
====


NOTE: when the jpeg geopackage tiles are generated, there is some interpolation and antialiasing going on, which makes part of the color not completely black, hence some pixel will remaining.
