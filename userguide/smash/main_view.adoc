...
If selected one tool, its icon becames red, this means that the tool is active. To deactivete it just tap the same icon in the bottom toolbar. The tools are automatically deactivated when the user decides to exit the bottom toolbar by tapping the handler on the right corner of the map view. In addition the feature editing tool will show its specific options in the bottom toolbar while activated.
...
* of all kind of features: point, lines, polygons and multipoint, multiline, multipolygon
* in their own coordinate reference system.

The features editing it works for all visible layers in a Geopackage format and allows to:
...
Editing of an existing feature consists of:
...
Now it is possible to either save the geometry changes image:images/version_1_3_0/39_editing.png[scaledwidth=5%, width=5%] or cancel image:images/version_1_3_0/40_editing.png[scaledwidth=5%, width=5%] to restore the initial geometry for that feature.
...
The second icon of the bottom toolbar activates the **Query layers** tool, that can be used to enable the vector layer info and attributes editing. Once enabled, tapping on the map will query all the visible vector layers for information in the tapped position. The attributes will be also editable if the selected feature is contained in a geopackage layer.
