...
There is also the possibility to visualize the GPS point count separated for the _valid_ and for _all_ points. And to decide to use the GPS filtered points (calculated applying the Kalman filter) to all GPS dependent features of the application (instead of the original raw data).
...
The CRS view contains the list of supporte CRS for the application with the possibility to new projections by epsg code. Available projections are used to reproject vector datasets (geopackage layers and shapefiles) onto the map.
